 ...
 <div data-aos="fade-up" class="row" style="margin-top: 30px; margin-bottom: 50px;">
    ...
    <div class="container mt-4">
      ...
      <div data-aos="fade-up" class="card bg-dark" style="margin-bottom: 100px; background-color: #23272a; width: 70%; margin:auto;">
        ...
        <form class="form-inline mt-4"  method = "POST" action = "chats.php">
          <?php
          ...
          ?>
            ...
            <div class="card-body">
              <h6><strong><?php echo ucwords($name);?></strong><a href="message.php?action=read&receiver=<?=$sent_by?>" class="btn btn-outline-warning" style = "float:right">Read message</a></h6>
              </form>
            </div>
          </div><br/>
          <?php
...
<div class="card-body mb-4">
  <h1 style="color:Tomato;">Students</h1>
  <?php
    if($_POST) {
      $sqlmain= "SELECT * FROM student WHERE fname ='$keyword' OR lname='$keyword' OR fname LIKE '$keyword%' OR lname LIKE '%$keyword' OR fname LIKE '%$keyword%'";
      } else {
      $sqlmain= "SELECT * FROM student ORDER BY lname ASC";
      }
    ?>
 <?php
    ...
    ';
        ...
        for($x=0; $x<$result->num_rows;$x++) {
          ...
          $receiver = $database->query("SELECT * FROM `student` WHERE `fname` = '$fname'") or die(mysqli_error());
          $r_receiver = $receiver->fetch_array();
          $email = $r_receiver["student_email"];
      ?>
...
<?php 
   if(isset($_GET["action"])){
    $action=$_GET["action"];
    if($action=='read')
      $sql1="UPDATE messages SET status = '$action'  WHERE received_by = '$useremail';";
      $database->query($sql1);                
}
?>
